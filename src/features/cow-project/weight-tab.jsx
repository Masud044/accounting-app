  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-16">
        ...
        <p className="text-sm text-gray-400">Loading weight history...</p>
      </div>
    );
  }
      ...
      <div className="rounded-lg bg-red-50 border border-red-200 p-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-red-600 mt-1">{error?.message || "Failed to load weight history."}</p>
            <Button 
              variant="outline" 
              size="sm" 
              onClick={() => refetch()} 
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end gap-2">
        ...
        >
          <IconCircleDashedPlus className="mr-1.5 h-4 w-4" /> Add Weight
        </Button>
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200">
        <Table>
          <TableHeader className="bg-gray-50">
            <TableRow>
              <TableHead className="text-xs font-semibold text-gray-500 uppercase tracking-wider py-2.5">Weight Date</TableHead>
              <TableHead className="text-right text-xs font-semibold text-gray-500 uppercase tracking-wider py-2.5">Weight</TableHead>
              <TableHead className="text-right text-xs font-semibold text-gray-500 uppercase tracking-wider py-2.5">Interval (days)</TableHead>
            ...
            {records.length ? (
              records.map((r) => (
                <TableRow key={r.ID} className="hover:bg-gray-50/70 transition-colors">
                  <TableCell className="font-medium text-sm text-gray-900 py-2.5">{formatDate(r.WEIGT_DATE)}</TableCell>
                  <TableCell className="text-right font-mono text-sm font-semibold text-gray-900 py-2.5">{r.WEIGHT ?? "—"}</TableCell>
                  <TableCell className="text-right font-mono text-sm text-gray-600 py-2.5">{r.INTERVAL_DAYS ?? "—"}</TableCell>
              ...
              <TableRow>
                <TableCell colSpan={4} className="h-32 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm font-medium text-gray-500">No Weight Records</p>
                    <p className="text-xs text-gray-400">Add a weight record to get started</p>
                  </div>
                </TableCell>
              </TableRow>
            )}
