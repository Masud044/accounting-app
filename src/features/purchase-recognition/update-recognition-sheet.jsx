...
// //  const isLocked = Number(formData?.STATUS) !== 1;
...
//   // ✅ STATUS 1 = Draft (editable). STATUS 2 = Waiting for Approval, STATUS 3 = Completed
//   //    Draft chara baki shob obosthay form locked/read-only thakbe
//   const isLocked = Number(formData?.STATUS) !== 1;
...
//             <div>
//               <SheetTitle>
//                 {isLocked ? "View" : "Edit"} Purchase Recognition {formId ? `— ${formId}` : ""}
//               </SheetTitle>
//               <SheetDescription>
//                 {statusNum === 2
//                   ? "This form is waiting for approval and is locked for editing."
//                   : statusNum === 3
//                   ? "This form is approved/completed and locked for editing."
//                   : "Update form details and line items"}
//               </SheetDescription>
//             </div>
//           </div>
//         </SheetHeader>
...
//                 <Lock className="h-3.5 w-3.5 shrink-0" />
//                 {statusNum === 2
//                   ? "This form is waiting for approval and cannot be edited right now."
//                   : "This form is approved/completed and locked for editing."}
//               </div>
//             )}
...
//               <Button onClick={handleSubmit} disabled={isSubmitting || items.length === 0 || isLocked}>
//                 {statusNum === 2
//                   ? <><Lock className="h-3.5 w-3.5 mr-1" />Waiting for Approval</>
//                   : statusNum === 3
//                   ? <><Lock className="h-3.5 w-3.5 mr-1" />Completed — Locked</>
//                   : isSubmitting
//                   ? <><Spinner className="mr-2 h-4 w-4" />Updating...</>
//                   : "Update Form"}
//               </Button>
...
import { Spinner } from "@/components/ui/spinner";
import { ArrowLeft, FileText, Trash2, Plus, Lock } from "lucide-react";
...
export default function EditRecognitionPage() {
  ...
  // STATUS 1 = Draft (editable). 2 = Waiting for Approval, 3 = Completed — dutoi locked
  const isLocked = Number(formData?.STATUS) !== 1;
  const statusNum = Number(formData?.STATUS);
      ...
      <div className="rounded-lg bg-white border border-gray-200 shadow-sm">
        ...
        {isLoading ? (
          ...
          <div className="p-5 space-y-5">
            {isLocked && (
              <div className="px-3 py-2 rounded-md bg-amber-50 text-amber-700 text-sm border border-amber-200 flex items-center gap-2">
                <Lock className="h-3.5 w-3.5 shrink-0" />
                {statusNum === 2
                  ? "This form is waiting for approval and cannot be edited right now."
                  : "This form is approved/completed and locked for editing."}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div className="space-y-1.5">
                ...
                <Input
                  type="date" value={header.recognitionDate}
                  onChange={(e) => updateHeader("recognitionDate", e.target.value)}
                  disabled={isDisabled} className="h-9 bg-white border-gray-200"
                />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <Input value={header.poNumber} disabled className="h-9 bg-gray-50 border-gray-200" />
              </div>
