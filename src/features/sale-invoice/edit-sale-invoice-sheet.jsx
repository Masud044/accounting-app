...
// import { Spinner } from "@/components/ui/spinner";
// import { Link } from "react-router-dom";
// import { FileText, Trash2, ChevronDown, Plus, X, Lock } from "lucide-react";
// import { useInvoiceById, useUpdateInvoice } from "./queries";
// import { useCustomers } from "@/features/customer/queries";
// import { useAvailableEggProductions } from "@/features/egg-production/queries";
...
//   // ── Lock state: RECEIVE_CREATED = 1 হলে পুরো ফর্ম view-only ────────────────
//   const isLocked   = Number(invoiceData?.RECEIVE_CREATED) === 1;
//   const isSubmitting = updateMutation.isPending;
...
//             : "Egg sale - daily production",
//           qty: totalQty,
//           saleQty: totalSaleQty || totalQty,   // ← fallback qty
//           unitPrice: rawLines[0]?.PRICE ?? "",
//         },
//       ]);
//     } else {
...
//   const handleSubmit = async () => {
//     if (isLocked) {
//       toast.error("This invoice is locked — Receive Voucher already created.");
//       return;
//     }
//     if (!customerId)        { toast.error("Please select a customer.");        return; }
//     if (!invoiceDate)       { toast.error("Please select invoice date.");      return; }
...
//   const handleCancel = async () => {
//     if (isDirty && !isLocked && showConfirmation) {
//       const ok = await showConfirmation({
//         title: "Discard changes?",
//         description: "You have unsaved changes. Close without saving?",
//         confirmText: "Discard", cancelText: "Keep Editing", variant: "destructive",
...
//                 Edit Invoice {invoiceData ? `#${invoiceData.HID}` : ""}
//                 {isLocked && (
...
//               <SheetDescription>
//                 {isLocked
//                   ? "Receive Voucher already created — view only, editing disabled."
...
//                     type="date" value={invoiceDate}
//                     onChange={(e) => { setInvoiceDate(e.target.value); setIsDirty(true); }}
//                     disabled={isReadOnly} className="h-9"
//                   />
//                 </div>
//               </div>
...
//                       type="button"
//                       className="h-9 shrink-0"
//                       disabled={checkedProdIds.length === 0 || isReadOnly}
//                       onClick={handleAddSelected}
//                     >
//                       <Plus className="h-3.5 w-3.5 mr-1" />
//                       Add {checkedProdIds.length > 0 ? `${checkedProdIds.length} ` : ""}Selected
...
//                   {/* Checked (not yet added) production date chips */}
//                   {checkedProdIds.length > 0 && (
//                     <div className="flex flex-wrap gap-2 pt-1">
//                       {checkedProdIds.map((id) => {
//                         const prod = productions.find((p) => String(p.ID) === id);
...
//               {/* Selected production date chips — visible in both edit & locked view */}
//               {lines.length > 0 && lines[0].components.length > 0 && (
//                 <div className="space-y-1.5">
//                   {isLocked && <Label className="text-xs font-medium">Production Dates</Label>}
//                   <div className="flex flex-wrap gap-2 pt-1">
//                     {lines[0].components.map((c) => {
//                       const displayDate = c.productionDate
//                         ? fmtDate(c.productionDate)
...
//                         <td colSpan={7} className="px-4 py-6 text-center text-sm text-muted-foreground">
//                           {isLocked
//                             ? "No production lines on this invoice."
//                             : "Select production record(s) above to add a line."}
//                         </td>
//                       </tr>
//                     )}
...
//                   disabled
//                   title="Receive Voucher already created for this invoice"
//                 >
...
import { Spinner } from "@/components/ui/spinner";
import { ArrowLeft, FileText, Trash2, ChevronDown, Plus, X, Lock } from "lucide-react";
import { useInvoiceById, useUpdateInvoice } from "./queries";
import { useCustomers } from "@/features/customer/queries";
import { useAvailableEggProductions } from "@/features/egg-production/queries";
...
function distributeQty(components, targetTotal) {
  const originalSum = components.reduce((s, c) => s + c.qty, 0) || 1;
  if (targetTotal === originalSum) return components.map((c) => c.qty);
  const raw = components.map((c) => (c.qty / originalSum) * targetTotal);
  const floored = raw.map(Math.floor);
  let remainder = targetTotal - floored.reduce((s, v) => s + v, 0);
  const order = raw
...
const editableCell =
  ...
  "focus-visible:ring-1 focus-visible:ring-primary/40 focus-visible:ring-offset-0 text-center px-1";

const highlightCell =
  "h-8 text-sm border-0 rounded-none bg-amber-50 " +
  "focus-visible:ring-1 focus-visible:ring-amber-400 focus-visible:ring-offset-0 text-center px-1";
...
export default function EditInvoicePage() {
  ...
  useEffect(() => {
    if (initialized || !invoiceData) return;
    ...
    setManualTotal("");
    setInitialized(true);
  }, [initialized, invoiceData]);

  const usedProdIds = lines.flatMap((l) => l.components.map((c) => String(c.productionId)));

  const goBack = () => navigate("/dashboard/sale-invoice");
  ...
  const handleAddSelected = () => {
    if (isReadOnly || checkedProdIds.length === 0) return;
    const chosen = productions.filter((p) => checkedProdIds.includes(String(p.ID)));
    if (chosen.length === 0) return;
    ...
    setLines((prev) => {
      if (prev.length > 0) {
        ...
        const newComponents = [
          ...existing.components,
          ...chosen.map((p) => ({ productionId: p.ID, qty: Number(p.QTY || 0), productionDate: p.PRODUCTION_DATE })),
        ];
        const totalQty = newComponents.reduce((s, c) => s + c.qty, 0);
        return [{
          ...existing,
      ...
      const totalQty = chosen.reduce((s, p) => s + Number(p.QTY || 0), 0);
      return [{
        components: chosen.map((p) => ({ productionId: p.ID, qty: Number(p.QTY || 0), productionDate: p.PRODUCTION_DATE })),
        date: today(),
        description:
          chosen.length > 1
            ? `Egg sale - ${chosen.length} production dates`
  ...
  const removeComponent = (prodId) => {
    ...
    setLines((prev) => {
      if (prev.length === 0) return prev;
      const line = prev[0];
      const newComponents = line.components.filter((c) => String(c.productionId) !== String(prodId));
      if (newComponents.length === 0) return [];
      const totalQty = newComponents.reduce((s, c) => s + c.qty, 0);
      return [{
        ...line,
  ...
  const subtotal = lines.reduce((s, l) => s + Number(l.saleQty || 0) * Number(l.unitPrice || 0), 0);
  const taxAmt = subtotal * (Number(taxRate || 0) / 100);
  const autoTotal = subtotal + taxAmt;
  const totalDue = manualTotal !== "" ? Number(manualTotal) : autoTotal;

  const availableProductions = productions.filter((p) => !usedProdIds.includes(String(p.ID)));

  const handleSubmit = async () => {
    if (isLocked) { toast.error("This invoice is locked — Receive Voucher already created."); return; }
    if (!customerId) { toast.error("Please select a customer."); return; }
    if (!invoiceDate) { toast.error("Please select invoice date."); return; }
    if (lines.length === 0) { toast.error("Add at least one production line."); return; }
    if (lines.some((l) => !l.unitPrice || Number(l.unitPrice) <= 0)) {
  ...
  const handleCancel = () => {
    if (isDirty && !isLocked && !window.confirm("You have unsaved changes. Discard and go back?")) return;
    goBack();
  };
  ...
  const isLoading = isLoadingInvoice || !initialized;
  ...
  return (
    <div className="p-4 md:p-6 pb-24">
      <div className="rounded-lg bg-white border border-gray-200 shadow-sm">
        ...
        {isLoading ? (
          ...
          <div className="p-5 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <CustomerPicker
                  customers={customers}
                  value={customerId}
                  onSelect={(v) => { setCustomerId(v); setIsDirty(true); }}
                  disabled={isReadOnly}
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  type="date" value={invoiceDate}
                  onChange={(e) => { setInvoiceDate(e.target.value); setIsDirty(true); }}
                  disabled={isReadOnly} className="h-9 bg-white border-gray-200"
                />
              </div>
            </div>
            ...
            {!isLocked && (
              <div className="space-y-1.5">
                <Label className="text-xs font-medium text-gray-500">Add Production Record</Label>
                <div className="flex gap-2">
                  <DropdownMenu open={pickerOpen} onOpenChange={setPickerOpen}>
                    <DropdownMenuTrigger asChild>
                      <Button
                        type="button" variant="outline"
                        className="h-9 flex-1 justify-between font-normal bg-white border-gray-200"
                        disabled={isReadOnly}
                      >
                        <span className="text-muted-foreground truncate">
                          {checkedProdIds.length > 0
                  ...
                  <div className="flex flex-wrap gap-2 pt-1">
                    {checkedProdIds.map((id) => {
                      ...
                      return (
                        <span
                          key={id}
                          className="inline-flex items-center gap-1.5 pl-3 pr-1.5 py-1 rounded-md bg-gray-100 text-gray-700 text-xs font-medium"
                        >
                          {prod ? fmtDate(prod.PRODUCTION_DATE) : `#${id}`}
                          <button
                            type="button"
            ...
            {/* Selected production date chips — both edit & locked view */}
            {lines.length > 0 && lines[0].components.length > 0 && (
              <div className="space-y-1.5">
                {isLocked && <Label className="text-xs font-medium text-gray-500">Production Dates</Label>}
                <div className="flex flex-wrap gap-2 pt-1">
                  {lines[0].components.map((c) => {
                    const displayDate = c.productionDate
                      ? fmtDate(c.productionDate)
                    ...
                    return (
                      <span
                        key={c.productionId}
                        className="inline-flex items-center gap-1.5 pl-3 pr-1.5 py-1 rounded-md bg-primary text-primary-foreground text-xs font-semibold"
                      >
                        {displayDate}
                        {!isLocked && (
                          <button
                            ...
                            onClick={() => removeComponent(c.productionId)}
                            disabled={isReadOnly}
                            className="rounded-sm p-0.5 hover:text-red-200"
                          >
                            <X className="h-3 w-3" />
                          </button>
                        )}
                  ...
                  {lines.length === 0 && (
                    <tr>
                      <td colSpan={7} className="px-4 py-6 text-center text-sm text-gray-400">
                        {isLocked ? "No production lines on this invoice." : "Select production record(s) above to add a line."}
                      </td>
                    </tr>
                  )}
                  ...
                  {lines.map((line, idx) => {
                    ...
                    return (
                      <tr key={idx}>
                        ...
                        <td className="px-3 py-1 w-28 text-center tabular-nums font-medium text-gray-700">
                          {Number(line.qty || 0).toLocaleString()}
                        </td>
                        <td className="px-1 py-1 w-28">
                          <Input
                            ...
                            onChange={(e) => updateLine(idx, "saleQty", e.target.value)}
                            disabled={isReadOnly}
                            className={highlightCell + " w-full"}
                          />
                        </td>
                        <td className="px-1 py-1 w-28">
                          <Input
                        ...
                        <td className="px-3 py-1 w-24 text-right tabular-nums font-semibold text-primary">
      ...
      {!isLoading && (
        <div className="fixed bottom-0 left-0 right-0 md:relative md:mt-4 flex flex-col sm:flex-row justify-end gap-2 px-6 py-4 bg-white border-t border-gray-200 md:border md:rounded-lg md:shadow-sm">
          ...
          {isLocked ? (
            <Button type="button" variant="secondary" disabled title="Receive Voucher already created for this invoice">
              Receive Voucher Created
            </Button>
          ) : (
            <Link to="/dashboard/receive-create" state={receiveVoucherState}>
