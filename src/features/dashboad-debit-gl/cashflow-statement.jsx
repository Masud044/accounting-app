...
const exportToExcel = (rows, fileName) => {
  const data = (rows ?? []).map((r) => ({
    "Root Account": r.ROOT_ACCOUNT === "ZZZ_TOTAL" ? "" : (r.ROOT_ACCOUNT ?? ""),
    "Account Name": r.ROOT_ACCOUNT === "ZZZ_TOTAL" ? "Total" : (r.ACCOUNT_NAME ?? ""),
    "Cash In (Debit)": Number(r.CASH_IN || 0),
    "Cash Out (Credit)": Number(r.CASH_OUT || 0),
    "Net Cash Flow": Number(r.NET_CASH_FLOW || 0),
  }));

  const ws = XLSX.utils.json_to_sheet(data.length ? data : [{}]);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "Cash Flow Summary");
...
export default function CashFlowReport() {
  ...
  const handleExport = () => {
    exportToExcel(rows, `Cash_Flow_Summary_${fromDate}_to_${toDate}.xlsx`);
  };
