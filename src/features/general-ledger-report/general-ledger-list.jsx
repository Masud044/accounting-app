  ...
  const handleExportExcel = () => {
    ...
    try {
      ...
      groups.forEach((g, gIdx) => {
        ...
        aoa.push(["Date", "Description", "Debit", "Credit", "Balance"]);
        aoa.push(["", "Opening Balance", "", "", fmtBalance(openingBal)]);
        ...
        aoa.push(["", "Total / Closing Balance", subtotal.debit, subtotal.credit, fmtBalance(closingBal)]);
      });

      if (grandTotals) {
        aoa.push([]);
        aoa.push(["Total Debit", grandTotals.debit]);
        aoa.push(["Total Credit", grandTotals.credit]);
        aoa.push(["Net Movement", `${fmt(Math.abs(net))} ${net < 0 ? "CR" : "DR"}`]);
      }

      const ws = XLSX.utils.aoa_to_sheet(aoa);
      ws["!cols"] = [
        { wch: 14 }, { wch: 42 }, { wch: 14 }, { wch: 14 }, { wch: 16 },
      ];
      ...
      XLSX.writeFile(wb, `general-ledger${suffix}.xlsx`);
    } finally {
      setIsExporting(false);
    }
  };
