  ...
  const columns = [
    {
      accessorKey: "VACCIN_NAME",
      header: () => (
        <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Vaccine</div>
      ),
      cell: ({ row }) => (
        <div className="font-semibold text-sm text-gray-900">{row.getValue("VACCIN_NAME")}</div>
      ),
    },
    {
      accessorKey: "DOSES",
      header: () => (
        <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Doses</div>
      ),
      cell: ({ row }) => (
        <div className="text-sm text-gray-600">{row.getValue("DOSES") || "—"}</div>
      ),
    },
    {
      accessorKey: "VACCIN_DATE",
      header: () => (
        <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Vaccination Date</div>
      ),
      cell: ({ row }) => (
        <div className="text-sm text-gray-700">{formatDate(row.getValue("VACCIN_DATE"))}</div>
      ),
    },
    {
      accessorKey: "NEXT_VACCIN_DATE",
      header: () => (
        <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Next Due</div>
      ),
      cell: ({ row }) => {
        const val = row.getValue("NEXT_VACCIN_DATE");
        const dueSoon = isDueSoon(val);
        return (
          <div className="flex items-center gap-2">
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-16">
        ...
        <p className="text-sm text-gray-400">Loading vaccination records...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        ...
        >
          <IconCircleDashedPlus className="mr-1.5 h-4 w-4" /> Add Vaccination
        </Button>
      </div>
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="h-32 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm font-medium text-gray-500">No vaccination records yet</p>
                    <p className="text-xs text-gray-400">Add the first vaccine record for this batch</p>
                  </div>
                </TableCell>
              </TableRow>
            )}
