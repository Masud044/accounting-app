...
import { useUpdateVaccination } from "./queries";
...
export default function UpdateVaccinationSheet({ open, onOpenChange, showConfirmation, record, hid }) {
  const updateMutation = useUpdateVaccination(hid);
  ...
  useEffect(() => {
    if (open && record) {
      form.reset({
        vaccinName:     record.VACCIN_NAME ?? "",
        doses:          record.DOSES ?? "",
        vaccinDate:     record.VACCIN_DATE ?? "",
        nextVaccinDate: record.NEXT_VACCIN_DATE ?? "",
      });
    }
  }, [open, record?.ID]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      toast.error(err?.message || "Failed to update vaccination record. Please try again.");
    }
  };
      ...
      <SheetContent className="sm:max-w-md w-full flex flex-col gap-0 p-0 z-105">
        <SheetHeader className="px-6 py-5 border-b border-border shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <SheetTitle>Update Vaccination</SheetTitle>
              <SheetDescription>Editing vaccination #{record?.ID}</SheetDescription>
            </div>
          </div>
        </SheetHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col flex-1 overflow-hidden">
            ...
            <div className="flex justify-end gap-2 px-6 py-4 border-t border-border shrink-0">
              ...
              <Button type="submit" disabled={isSubmitting}>
                {isSubmitting ? <><Spinner className="mr-2 h-4 w-4" />Updating...</> : "Update Vaccination"}
              </Button>
            </div>
          </form>
        </Form>
