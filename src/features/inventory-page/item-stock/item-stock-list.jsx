...
import { useItemStocks, useDeleteItemStock, useUpdateItemStock } from "./queries";
import AddItemStockSheet from "./add-item-stock-sheet";
import UpdateItemStockSheet from "./update-item-stock-sheet";
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <Button
                size="sm"
                variant="outline"
                className="flex-1 h-7 text-xs bg-white border-gray-200"
                onClick={() => setOpen(false)}
                disabled={loading}
              >
                Cancel
...
export default function ItemStockList() {
  ...
  const columns = [
    ...
    {
      accessorKey: "STORE_NAME",
      header: ({ column }) => <SortableHeader column={column} label="Store Name" />,
      cell: ({ row }) => (
        <div className="ml-3 text-sm text-gray-500">{row.getValue("STORE_NAME")}</div>
      ),
    },
    {
      accessorKey: "ITEM_NAME",
      header: ({ column }) => <SortableHeader column={column} label="Item Name" />,
      cell: ({ row }) => (
        <div className="ml-3 text-sm font-medium text-gray-800">{row.getValue("ITEM_NAME") || "—"}</div>
      ),
    },
    {
      accessorKey: "STOCK_QTY",
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="ml-3 flex items-center gap-1">
            <span className={`text-sm font-medium ${isLow ? "text-red-600" : "text-gray-800"}`}>
              {qty ?? "—"}
            </span>
            {isLow && (
    ...
    },
    {
      accessorKey: "MINIMUM_LEVEL",
      header: () => (
        <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Min Level</div>
      ),
      cell: ({ row }) => (
        <MinLevelCell
          stock={row.original}
    ...
    },
    {
      accessorKey: "STOCK_PRICE",
      header: () => (
        <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Price</div>
      ),
      cell: ({ row }) => {
        const p = row.getValue("STOCK_PRICE");
        return <div className="text-sm text-gray-600">{p != null ? Number(p).toFixed(2) : "—"}</div>;
      },
    },
    {
      accessorKey: "UOM",
      header: () => (
        <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide">UOM</div>
      ),
      cell: ({ row }) => <div className="text-sm text-gray-600">{row.getValue("UOM") || "—"}</div>,
    },
    {
      accessorKey: "STOCK_STATUS",
      header: () => (
      ...
      <div className="rounded-lg bg-white border border-gray-200 shadow-sm">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-5 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-sm font-bold text-gray-900">Item Stock</h2>
              <p className="text-xs text-gray-400">{stocks.length} total stock records</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
