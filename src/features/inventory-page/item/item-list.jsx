...
export default function ItemList() {
  ...
  const itemNameOptions = useMemo(() => {
    const names = new Set(
      items.map((it) => it.NAME).filter((n) => n && String(n).trim())
    );
    return Array.from(names).sort();
  }, [items]);

  // ── Apply item name filter before passing data to the table ──────────────
  const filteredItems = useMemo(() => {
    if (itemNameFilter === "all") return items;
    return items.filter((it) => it.NAME === itemNameFilter);
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="ml-3">
          ...
          {row.original.MODEL && (
            <div className="text-xs text-gray-400">{row.original.MODEL}</div>
          )}
        </div>
      ),
    },
    {
      accessorKey: "TYPE_NAME",
      header: () => (
        <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Type Name</div>
      ),
      cell: ({ row }) => (
    ...
    },
    {
      accessorKey: "UNIT",
      header: () => (
        <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Unit</div>
      ),
      cell: ({ row }) => <div className="text-sm text-gray-600">{row.getValue("UNIT") || "—"}</div>,
    },
    {
      accessorKey: "MIN_LEVEL",
      header: () => (
      ...
      cell: ({ row }) => <div className="text-sm text-gray-600">{row.getValue("MIN_LEVEL") ?? "—"}</div>,
    },
    {
      accessorKey: "STATUS",
      header: () => (
    ...
    <div className="p-4 md:p-6">
      <div className="rounded-lg bg-white border border-gray-200 shadow-sm">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-5 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">{filteredItems.length} of {items.length} items</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="p-5 space-y-4">
          {/* Search + Item Name filter + Column visibility */}
