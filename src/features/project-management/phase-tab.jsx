...
import { usePhases, useDeletePhase } from "./queries";
...
export default function PhasesTab({ projectId }) {
  ...
  const { data: records = [], isLoading } = usePhases(projectId);
  ...
  const handleDelete = async (record) => {
    const confirmed = await showConfirmation({
      title: "Delete phase?",
      description: `Are you sure you want to delete "${record.PHASE_NAME}"? Its activities will also be removed. This action cannot be undone.`,
      confirmText: "Delete",
      cancelText: "Cancel",
      variant: "destructive",
    });
            ...
            {table.getRowModel().rows?.length ? (
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="h-24 text-center">
                  <Empty>
                    <EmptyHeader>
                      <EmptyMedia variant="icon"><Milestone /></EmptyMedia>
