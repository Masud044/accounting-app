...
// import ActivityLogSheet from "./add-activity-log-sheet";
...
//   header: "Month",
//   cell: ({ row }) => <div>{MONTH_NAMES[Number(row.getValue("ACTIVITY_MONTH"))] || "—"}</div>,
// },
//     {
...
// <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleOpenLogs(record)}>
...
import { Trash2, ListChecks, NotebookPen } from "lucide-react";
import { toast } from "react-toastify";
import { IconCircleDashedPlus, IconEdit } from "@tabler/icons-react";
...
export default function ActivitiesTab({ calendarId }) {
  ...
  const handleOpenLogs = (record) => {
    navigate(`/dashboard/farm-activity-log/detail/${record.DETAIL_ID}`);
  };
