...
const defaultValues = { kpiName: "", farmType: "", targetValue: "", unit: "", actualValue: "", remarks: "" };
...
export default function AddKpiSheet({ open, onOpenChange, showConfirmation, calendarId }) {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      toast.error(err?.message || "Failed to add KPI target. Please try again.");
    }
  };
      ...
      <SheetContent className="sm:max-w-md w-full flex flex-col gap-0 p-0 z-105">
        <SheetHeader className="px-6 py-5 border-b border-border shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <SheetTitle>Add KPI Target</SheetTitle>
              <SheetDescription>Set a KPI target for this calendar</SheetDescription>
            </div>
          </div>
        </SheetHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col flex-1 overflow-hidden">
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
              <FormField control={form.control} name="kpiName" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input placeholder="e.g. Mortality rate" disabled={isSubmitting} {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              ...
              <FormField control={form.control} name="unit" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input placeholder="e.g. %" disabled={isSubmitting} {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              ...
              <FormField control={form.control} name="remarks" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Textarea rows={2} placeholder="Optional" disabled={isSubmitting} {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
            </div>
            ...
            <div className="flex justify-end gap-2 px-6 py-4 border-t border-border shrink-0">
              ...
              <Button type="submit" disabled={isSubmitting}>
                {isSubmitting ? <><Spinner className="mr-2 h-4 w-4" />Adding...</> : "Add KPI Target"}
              </Button>
            </div>
          </form>
        </Form>
