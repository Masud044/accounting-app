...
import { toast } from "react-toastify";
import { IconFileSpreadsheet, IconLoader2, IconX } from "@tabler/icons-react";
import { TrendingUp, TrendingDown, Wallet } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
...
const IncomeExpenseReport = () => {
  ...
  const handleExportExcel = () => {
    ...
    const sheetTitle = isExpense ? "Expense Statement" : "Income Statement";

    const exportData = rows.map((r) => ({
      "Voucher No": r.VOUCHERNO,
      "Date": r.GL_ENTRY_DATE
        ? new Date(r.GL_ENTRY_DATE).toLocaleDateString("en-US", {
            month: "short",
            day: "2-digit",
            year: "numeric",
        ...
        : "",
      "Description": r.DESCRIPTION_WITH_CODE,
    ...
    ws["!cols"] = [{ wch: 16 }, { wch: 14 }, { wch: 40 }, { wch: 16 }];
    ...
    const suffix = isFiltered
      ? `_${applied.fromDate || "all"}_to_${applied.toDate || "all"}`
      : "_all";
    XLSX.writeFile(wb, `${reportType}-statement${suffix}.xlsx`);
  };
